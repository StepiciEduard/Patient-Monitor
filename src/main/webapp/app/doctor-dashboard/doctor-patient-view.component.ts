...
import { DashboardData, Reading } from '../patient-dashboard/patient-dashboard.service';
...
@Component({
  selector: 'jhi-doctor-patient-view',
  standalone: true,
  imports: [CommonModule, RouterModule],
  templateUrl: './doctor-patient-view.component.html',
  styleUrl: './doctor-patient-view.component.scss',
})
export default class DoctorPatientViewComponent implements OnInit, OnDestroy {
  ...
  loadData(): void {
    this.loading = true;
    this.error = false;
    this.dashboardService.getPatientDashboardData(this.patientId, this.selectedInterval).subscribe({
      next: data => {
        this.data = data;
  ...
  private createCharts(): void {
    ...
    for (const config of this.vitalConfigs) {
      const canvas = document.getElementById(`doc-chart-${config.key}`) as HTMLCanvasElement;
      if (!canvas) continue;
