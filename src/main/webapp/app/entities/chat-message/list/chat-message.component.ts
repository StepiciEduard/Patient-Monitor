...
import { ChatMessageService, EntityArrayResponseType } from '../service/chat-message.service';
...
export class ChatMessageComponent implements OnInit {
  ...
  protected readonly chatMessageService = inject(ChatMessageService);
  ...
  trackId = (item: IChatMessage): number => this.chatMessageService.getChatMessageIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.chatMessages.set(dataFromBody);
  }
  ...
  protected fillComponentAttributesFromResponseBody(data: IChatMessage[] | null): IChatMessage[] {
    ...
    if (this.links().prev) {
      ...
      if (data) {
        for (const d of data) {
          if (chatMessagesNew.some(op => op.id === d.id)) {
